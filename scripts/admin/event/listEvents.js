...
function createHorizontalCard(data) {
    ...
    regStartDateElement.textContent = 'Registration Start Date: ' + new Date(data.regStartDate).toLocaleString();

    const regEndDateElement = document.createElement('p');
    regEndDateElement.className = 'card-text';
    regEndDateElement.textContent = 'Registration End Date: ' + new Date(data.regEndDate).toLocaleString();

    const eventDateElement = document.createElement('p');
    eventDateElement.className = 'card-text';
    eventDateElement.textContent = 'Event Date: ' + new Date(data.eventDate).toLocaleString();
    ...
    linkElement.href = `/admin/events/addParticipant?participantDocId=${data.participantDocId}&eventId=${data._id}`;
...
if (events) {
  ...
  document.getElementById("no-events-box").classList.remove("d-none")
}
